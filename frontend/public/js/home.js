...
(function () {
  ...
  function renderLuxuryProductCard(p) {
    const secondImg = (p.images && p.images[1]) || p.image;
  ...
  function initGSAP() {
    ...
    /* Animate position only — never opacity — so hero never stays invisible if GSAP hiccups */
    gsap.from(".lux-hero-copy > *", {
      y: 32,
      duration: 0.9,
      stagger: 0.12,
      ease: "power3.out",
  ...
  function initCartListener() {
    window.addEventListener("vedvigyan:cart-updated", renderProducts);
  }
  ...
  function init() {
    ...
    document.querySelectorAll(".lux-hero-copy > *, .lux-hero-visual").forEach((el) => {
      el.style.opacity = "1";
