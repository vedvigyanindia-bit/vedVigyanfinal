function renderGemstonePage() {
  const host = document.getElementById('gemStoneCatalog');
  if (!host) return;

  const data = window.VED_VIGYAN_DATA;
  const products = (data?.products || []).filter(p => p.category === 'gem-stone' || p.category === 'gemstone-tree' || p.category === 'crystal-bracelet' || p.category === 'zodiac-bracelet' || p.category === 'bracelet');
